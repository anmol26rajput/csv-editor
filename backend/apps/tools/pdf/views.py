...
from .text_editor import get_text_spans, apply_text_edits
from .serializers import (
    PDFMergeSerializer, PDFSplitSerializer, PDFReorderSerializer,
    PDFConvertSerializer, PDFToPDFSerializer, PDFTextEditSerializer,
)
...
class PDFTextSpansView(APIView):
    """Text spans (with geometry) for one page, for the inline text editor."""
    def get(self, request, file_id):
        ...
        try:
            ...
            return Response(get_text_spans(doc.file.path, page_number))
        except ValueError as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

class PDFEditTextView(APIView):
    """Apply inline text edits and return the modified PDF as a new document."""
    @extend_schema(request=PDFTextEditSerializer)
    def post(self, request):
        serializer = PDFTextEditSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        ...
        temp_dir = os.path.join(settings.MEDIA_ROOT, 'temp', f"textedit_{doc.id}")
        os.makedirs(temp_dir, exist_ok=True)
        try:
            base, _ = os.path.splitext(doc.filename)
            output_filename = f"{base}_edited.pdf"
            output_path = os.path.join(temp_dir, output_filename)
            apply_text_edits(doc.file.path, serializer.validated_data['edits'], output_path)
